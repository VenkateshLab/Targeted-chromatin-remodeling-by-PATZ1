...
inputFiles <- c("Uninjured/fragments.tsv.gz","7dpi/fragments.tsv.gz")

# Define sample names corresponding to each file
sampleNames <- c("Uninjured","Injured_7dpi")
```
...
proj <- ArchRProject(
  ...
  outputDirectory = "Uninjured_vs_Injured7dpi_1000_4",
...
```{r}
ggsave("Uninjured_vs_Injured7dpi_1000_4/QC_fragments_metrics.png", plot = p, width = 5, height = 5, dpi = 300, device = "png")


ggsave("Uninjured_vs_Injured7dpi_1000_4/QC_FragmentSizes_metrics.png", plot = p3, width = 7, height = 4, dpi = 300, device = "png")

```


```{r}
#saveArchRProject(ArchRProj = proj, outputDirectory = "Uninjured_vs_Injured7dpi_1000_4", load = FALSE)
```
...
ggsave("Uninjured_vs_Injured7dpi_1000_4/Uninjured_vs_Injured7dpi_sample_cluster.png", plot = p5, width = 5, height = 5, dpi = 300, device = "png")
# Save p2
#ggsave("p2_all.pdf", plot = p2, width = 5, height = 5,dpi = 300,units = "in")
ggsave("Uninjured_vs_Injured7dpi_1000_4/Uninjured_vs_Injured7dpi_orginal_cluster.png", plot = p6, width = 5, height = 5, dpi = 300, device = "png")
```
...
write.csv(markerList,"Uninjured_vs_Injured7dpi_1000_4/Uninjured_vs_Injured7dpi_marker_gene_Sample.csv")
```
...
write.csv(peak_list,"Uninjured_vs_Injured7dpi_1000_4/Peak_List_Only_CST.csv",row.names = F)
```
```{r}
...
```{r}
# Load the data
peak_list_data <- read.csv("Uninjured_vs_Injured7dpi_1000_4/Peak_List_Only_CST.csv")
...
# Get counts for Uninjured
uninjured_counts <- count_peak_types("Uninjured")
# Print the results
print("Peak Type Counts for Injured_7dpi")
print(injured_7dpi_counts)
print("Peak Type Counts for Uninjured")
print(uninjured_counts)
```
```{r}
...
```
```{r}
pma <- plotMarkers(seMarker = markerTest, name = "Uninjured", cutOff = "Pval <= 0.05 & abs(Log2FC) >= 0.5", plotAs = "MA")
pma
```
...
element_metadata <- elementMetadata(Uninjured)
seqnames <- as.character(element_metadata$seqnames)
start <- element_metadata$start
end <- element_metadata$end
# Extract assay data
log2FC <- Uninjured@assays@data@listData$Log2FC$x
mean <- Uninjured@assays@data@listData$Mean$x
fdr <- Uninjured@assays@data@listData$FDR$x
pval <- Uninjured@assays@data@listData$Pval$x
meanDiff <- Uninjured@assays@data@listData$MeanDiff$x
auc <- Uninjured@assays@data@listData$AUC$x
meanBGD <- Uninjured@assays@data@listData$MeanBGD$x
...
if (all(lengths == lengths[1])) {
  ...
  write.csv(peak_data, "Peak_Matrix_Pairwise_Uninjured_vs_Injured.csv", row.names = FALSE)
} else {
...
# Load your data
peak_data <- read.csv("Peak_Matrix_Pairwise_Uninjured_vs_Injured.csv")
...
write.csv(final_df, "Peak_Matrix_Pairwise_Uninjured_vs_Injured_Annotated.csv",row.names = F)
```
...
if (all(lengths == lengths[1])) {
  ...
  # Write the data frame to a CSV file
  write.csv(peak_data, "Peak_Matrix_Pairwise_Injured_vs_Uninjured.csv", row.names = FALSE)
} else {
  stop("The lengths of the extracted data do not match.")
}
```
...
library(TxDb.Mmusculus.UCSC.mm10.knownGene)
# Load your data
peak_data <- read.csv("Peak_Matrix_Pairwise_Injured_vs_Uninjured.csv")
# Rename columns to avoid conflicts
colnames(peak_data)[which(colnames(peak_data) %in% c("seqnames", "start", "end"))] <- c("chrom", "chromStart", "chromEnd")
# Create a GRanges object
gr <- GRanges(seqnames = Rle(peak_data$chrom),
...
final_df <- annotated_full_df[, selected_columns]
# Save the annotated data to a new CSV file
write.csv(final_df, "Peak_Matrix_Pairwise_Injured_vs_Uninjured_Annotated.csv",row.names = F)
```